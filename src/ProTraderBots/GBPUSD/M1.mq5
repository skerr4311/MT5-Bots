//+--------------------------------------------------------------------+
//| Copyright:  (C) 2023 Forex Software Ltd.                           |
...
#property copyright "Copyright (C) 2023 Forex Software Ltd."
...
static input double Maximum_Amount  = 0.01; // Maximum position amount [lot]
...
input int Slot0IndParam0 = 32; // Period
input int Slot0IndParam1 = 0; // Shift
static input string IndicatorName1 = "RSI"; // ------ Indicator parameters ------
input int Slot1IndParam0 = 42; // Smoothing period
input int Slot1IndParam1 = 53; // Level
static input string IndicatorName2 = "Moving Averages Crossover"; // ------ Indicator parameters ------
input int Slot2IndParam0 = 11; // Fast MA period
input int Slot2IndParam1 = 9; // Slow MA period
...
input int Slot5IndParam0 = 250; // Stop Loss
input int Slot5IndParam1 = 600; // Take Profit
...
enum BandIndLogic
  {
   ...
   BandIndLogic_The_bar_closes_below_Upper_Band_after_above,
   BandIndLogic_The_bar_closes_above_Upper_Band_after_below,
   BandIndLogic_The_bar_closes_below_Lower_Band_after_above,
   BandIndLogic_The_bar_closes_above_Lower_Band_after_below,
...
protected:
   ...
   void              BarClosesAboveIndicatorAfterClosingBelowLogic(int firstBar,int previous,const double &adIndValue[],
                                                                  IndicatorComp &indCompLong,IndicatorComp &indCompShort);

   void              BarClosesBelowIndicatorAfterClosingAboveLogic(int firstBar,int previous,const double &adIndValue[],
                                                                  IndicatorComp &indCompLong,IndicatorComp &indCompShort);
   ...
   switch(indLogic)
     {
      ...
      case BandIndLogic_The_bar_closes_below_Upper_Band_after_above:
         for(int bar=firstBar; bar<Data.Bars; bar++)
           {
            int baseBar=bar-1;
            while(MathAbs(Data.Close[baseBar]-adUpperBand[baseBar-previous])<sigma && baseBar>firstBar)
               baseBar--;

            indCompLong.Value[bar]=Data.Close[bar]<adUpperBand[bar-previous]-sigma && 
                                   Data.Close[baseBar]>adUpperBand[baseBar-previous]+sigma ? 1 : 0;

            baseBar=bar-1;
            while(MathAbs(Data.Close[baseBar]-adLowerBand[baseBar-previous])<sigma && baseBar>firstBar)
               baseBar--;

            indCompShort.Value[bar]=Data.Close[bar]>adLowerBand[bar-previous]+sigma && 
                                    Data.Close[baseBar]<adLowerBand[baseBar-previous]-sigma ? 1 : 0;
           }
         break;

      case BandIndLogic_The_bar_closes_above_Upper_Band_after_below:
         for(int bar=firstBar; bar<Data.Bars; bar++)
           {
            int baseBar=bar-1;
            while(MathAbs(Data.Close[baseBar]-adUpperBand[baseBar-previous])<sigma && baseBar>firstBar)
      ...
      case BandIndLogic_The_bar_closes_below_Lower_Band_after_above:
         for(int bar=firstBar; bar<Data.Bars; bar++)
           {
            int baseBar=bar-1;
            while(MathAbs(Data.Close[baseBar]-adLowerBand[baseBar-previous])<sigma && baseBar>firstBar)
            ...
            indCompLong.Value[bar]=Data.Close[bar]<adLowerBand[bar-previous]-sigma && 
                                   Data.Close[baseBar]>adLowerBand[baseBar-previous]+sigma ? 1 : 0;
            ...
            indCompShort.Value[bar]=Data.Close[bar]>adUpperBand[bar-previous]+sigma && 
                                    Data.Close[baseBar]<adUpperBand[baseBar-previous]-sigma ? 1 : 0;
           }
         break;

      case BandIndLogic_The_bar_closes_above_Lower_Band_after_below:
         for(int bar=firstBar; bar<Data.Bars; bar++)
           {
            int baseBar=bar-1;
            while(MathAbs(Data.Close[baseBar]-adLowerBand[baseBar-previous])<sigma && baseBar>firstBar)
...
//|                                                                  |
//+------------------------------------------------------------------+
void Indicator::BarClosesAboveIndicatorAfterClosingBelowLogic(int firstBar,int previous,const double &adIndValue[],
                                                             IndicatorComp &indCompLong,IndicatorComp &indCompShort)
  {
   double sigma=Sigma();
   firstBar=MathMax(firstBar,2);
   ...
   for(int bar=firstBar; bar<Data.Bars; bar++)
     {
      int baseBar=bar-1;
      while(MathAbs(Data.Close[baseBar]-adIndValue[baseBar-previous])<sigma && baseBar>firstBar)
         baseBar--;

      indCompLong.Value[bar]=Data.Close[bar]>adIndValue[bar-previous]+sigma && 
                             Data.Close[baseBar]<adIndValue[baseBar-previous]-sigma ? 1 : 0;
      indCompShort.Value[bar]=Data.Close[bar]<adIndValue[bar-previous]-sigma && 
                              Data.Close[baseBar]>adIndValue[baseBar-previous]+sigma ? 1 : 0;
     }
  }
//+------------------------------------------------------------------+
//|                                                                  |
//+------------------------------------------------------------------+
void Indicator::BarClosesBelowIndicatorAfterClosingAboveLogic(int firstBar,int previous,const double &adIndValue[],
                                                             IndicatorComp &indCompLong,IndicatorComp &indCompShort)
  {
   double sigma=Sigma();
   firstBar=MathMax(firstBar,2);
   ...
   if(SlotType==SlotTypes_OpenFilter || SlotType==SlotTypes_CloseFilter)
     {
      ...
      else if(ListParam[0].Text=="Moving Average changes its direction upward")
         IndicatorChangesItsDirectionUpward(firstBar,previous,movingAverage,Component[1],Component[2]);
      else if(ListParam[0].Text=="Moving Average changes its direction downward")
         IndicatorChangesItsDirectionDownward(firstBar,previous,movingAverage,Component[1],Component[2]);
      ...
      else if(ListParam[0].Text=="The bar closes above Moving Average after closing below it")
         BarClosesAboveIndicatorAfterClosingBelowLogic(firstBar,previous,movingAverage,Component[1],Component[2]);
      else if(ListParam[0].Text=="The bar closes below Moving Average after closing above it")
         BarClosesBelowIndicatorAfterClosingAboveLogic(firstBar,previous,movingAverage,Component[1],Component[2]);
     }
  }
//+------------------------------------------------------------------+
         ...
         if(IsLongerTimeFrame(i))
           {
            ...
            if(isBasePriceOpen)
			 ...
			 string logicRule = Slot[i].IndicatorPointer.ListParam[0].Text;
			 StringToLower(logicRule);
             if (StringFind(logicRule, "bar closes") >= 0)
			   {
				 ltfShift = 1;
				 isCloseFilterShift = true;
			   }
...
void Strategy::DynamicInfoInitArrays(string &params[],string &values[])
  {
   ArrayResize(params,300);
   ArrayResize(values,300);
   for(int i=0; i<300; i++)
     {
      params[i] = "";
      values[i] = "";
...
Strategy *StrategyManager::GetStrategy(void)
  {
   ...
   strategy.Description            = "Exported on 19.9.2024 г. from Forex Strategy Builder Professional, v4.3.3";
   strategy.RecommendedBars        = 1000;
   strategy.FirstBar               = 45;
   strategy.MinBarsRequired        = 48;
         ...
         if(PositionSelect(_Symbol))
           {
            ...
            bool isOrderCheck=ActionTrade::CheckOrder(request);
            ...
            if(isOrderCheck)
              {
               ...
               if(!isOrderSend)
                  Print("Error: ",__FUNCTION__,"(): OrderSend(): ",ResultRetcodeDescription(result.retcode));
              }
...
//|                                                                  |
//+------------------------------------------------------------------+
bool ActionTrade::CheckOrder(MqlTradeRequest &request)
{
         ...
         if(IsTradeContextFree())
           {
            ...
            bool isExecuted=isOrderCheck && isOrderSend && result.retcode==TRADE_RETCODE_DONE;
            string retcode=ResultRetcodeDescription(result.retcode);
